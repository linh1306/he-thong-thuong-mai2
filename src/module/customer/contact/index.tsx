...
import { createContact } from "@/utils/api/customer/contact";
...
export default function PageContact() {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const res  = await createContact({ name, email, phone, description })
      if(res.status === 'success'){
        form.resetFields()
      }
      createNotification(res.status,res.message)
      console.log({ name, email, phone, description },res);
