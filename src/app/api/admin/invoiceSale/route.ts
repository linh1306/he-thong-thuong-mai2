...
export async function GET(req: Request) {
  ...
  try {
    ...
    try {
      const res = await InvoiceSale.find(removeEmptyFields(options))
        .skip((currentPage - 1) * pageSize)
        .limit(pageSize)

      const totalDocuments = await InvoiceSale.countDocuments(removeEmptyFields(options))
...
export async function PUT(req: Request) {
  try {
    ...
    const res = await InvoiceSale.updateOne({ _id: _user }, { statusInvoice })
